...
const { GATSBY_IS_MOBILE } = process.env;
// const { toast } = require("react-toasify");
...
function getFormattedDate(gmtDate) {
    // Parse the GMT date-time string
    const gmtDateTime = dayjs.utc(gmtDate);

    // Convert to the browser's local time
    const localDateTime = gmtDateTime.local();

    // Extract date components
    const date = localDateTime.format("Do");
    const month = localDateTime.format("MMMM"); // Month name
